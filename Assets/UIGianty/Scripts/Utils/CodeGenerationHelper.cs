...
#region TAGS and REGIONS
	...
	static public string GenerateScript (string modelName, string baseType, params CustomPropertyInfo[] properties) {
		...
		if (text != null) {
			...
			Logger.LogError("There are something wrong, could not find code template!");
		}
...
public class CustomPropertyInfo {
	...
	public T GetLastValueAs<T> () {
		try {
			...
			Logger.Log("Type of property has been changed, the default value will set to default value of new type!");
			return default(T);
		}
	}
